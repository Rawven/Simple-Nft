...
func RankAdd() job.XxlTaskFunc {
	return job.XxlTaskFunc{
		...
		Task: func(cxt context.Context, param *xxl.RunReq) string {
			...
			// 获取所有搜索热度和购买热度
			buyResult, err := red.HGetAll(ctx, nft.RankAddBuy).Result()
			if err != nil {
				logger.Error("获取购买热度失败", err)
				return "获取购买热度失败"
			}

			searchResult, err := red.HGetAll(ctx, nft.RankAddClick).Result()
			if err != nil {
			...
			for k, v := range buyResult {
				...
				pipe.HIncrBy(ctx, nft.RankAddBuy, k, -buyValue)
			}
			...
			for k, v := range searchResult {
				...
				pipe.HIncrBy(ctx, nft.RankAddClick, k, -searchValue)
			}
